...
def decrypt_and_parse_hex(hex_string: str) -> dict:
    """Decrypts an AES-CBC encrypted hex payload and parses its protobuf structure into a dictionary."""
    decrypted_bytes = aes_decrypt(hex_string)
    proto_json = get_available_room(decrypted_bytes.hex())
    if proto_json:
